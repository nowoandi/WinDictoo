...
STRINGS: dict[str, dict[str, str]] = {
    ...
    "rec.model_missing": {
        "ru": "{model} ещё нет на диске — {size} МБ скачаются при загрузке",
        "en": "{model} is not on disk yet — {size} MB will be downloaded on load",
        "de": "{model} liegt noch nicht vor — {size} MB werden beim Laden heruntergeladen",
        "fr": "{model} n'est pas encore sur le disque — {size} Mo seront téléchargés au chargement",
        "es": "{model} aún no está en el disco: se descargarán {size} MB al cargar",
        "zh": "{model} 尚未下载到本地 — 加载时将下载 {size} MB",
        "tr": "{model} henüz diskte yok — yükleme sırasında {size} MB indirilecek",
        "hy": "{model}-ը դեռ սկավառակի վրա չէ — բեռնման ժամանակ կներբեռնվի {size} ՄԲ",
    },
    "rec.model_download_failed": {
        "ru": "Не удалось скачать модель {model} ({size} МБ). Проверьте подключение к интернету.",
        "en": "Could not download {model} ({size} MB). Check your internet connection.",
        "de": "{model} ({size} MB) konnte nicht heruntergeladen werden. Prüfen Sie die Internetverbindung.",
        "fr": "Impossible de télécharger {model} ({size} Mo). Vérifiez votre connexion Internet.",
        "es": "No se pudo descargar {model} ({size} MB). Comprueba tu conexión a Internet.",
        "zh": "无法下载 {model}（{size} MB）。请检查网络连接。",
        "tr": "{model} ({size} MB) indirilemedi. İnternet bağlantınızı kontrol edin.",
        "hy": "Չհաջողվեց ներբեռնել {model}-ը ({size} ՄԲ)։ Ստուգեք ինտերնետ կապը։",
    },
    "rec.btn_load_now": {
    ...
    "ref.cloud_blocked": {
        "ru": "⚠ Модель «{name}» — облачная: Ollama отправит текст на свои серверы. Улучшение с ней отключено, вставляется исходная расшифровка. Возьмите модель без «:cloud».",
        "en": "⚠ The model \"{name}\" is cloud-hosted: Ollama would send your text to its servers. Refinement with it is disabled and the raw transcript is used. Pick a model without \":cloud\".",
        "de": "⚠ Das Modell „{name}“ läuft in der Cloud: Ollama würde Ihren Text an seine Server senden. Die Verbesserung damit ist deaktiviert, es wird die Rohtranskription verwendet. Wählen Sie ein Modell ohne „:cloud“.",
        "fr": "⚠ Le modèle « {name} » est hébergé dans le cloud : Ollama enverrait votre texte à ses serveurs. L'amélioration avec ce modèle est désactivée, la transcription brute est utilisée. Choisissez un modèle sans « :cloud ».",
        "es": "⚠ El modelo «{name}» está alojado en la nube: Ollama enviaría tu texto a sus servidores. La mejora con él está desactivada y se usa la transcripción original. Elige un modelo sin «:cloud».",
        "zh": "⚠ 模型「{name}」在云端运行：Ollama 会把你的文本发送到它的服务器。已禁用该模型的文本改进，将使用原始识别结果。请选择不带「:cloud」的模型。",
        "tr": "⚠ \"{name}\" modeli bulutta çalışıyor: Ollama metninizi kendi sunucularına gönderir. Bu modelle iyileştirme devre dışı, ham çözümleme kullanılır. \":cloud\" içermeyen bir model seçin.",
        "hy": "⚠ «{name}» մոդելը ամպային է․ Ollama-ն ձեր տեքստը կուղարկի իր սերվերներ։ Դրանով բարելավումն անջատված է, օգտագործվում է սկզբնական վերծանումը։ Ընտրեք «:cloud»-ից զուրկ մոդել։",
    },
    "ref.cloud_in_list": {
        "ru": "⚠ Облачные модели ({names}) отключены — они отправляют текст наружу.",
        "en": "⚠ Cloud models ({names}) are disabled — they send your text off the machine.",
        "de": "⚠ Cloud-Modelle ({names}) sind deaktiviert — sie senden Ihren Text nach außen.",
        "fr": "⚠ Les modèles cloud ({names}) sont désactivés — ils envoient votre texte à l'extérieur.",
        "es": "⚠ Los modelos en la nube ({names}) están desactivados: envían tu texto fuera del equipo.",
        "zh": "⚠ 云端模型（{names}）已禁用——它们会把文本发送到外部。",
        "tr": "⚠ Bulut modelleri ({names}) devre dışı — metninizi makine dışına gönderirler.",
        "hy": "⚠ Ամպային մոդելներն ({names}) անջատված են՝ դրանք տեքստն ուղարկում են դուրս։",
    },
    "ref.no_models": {
    ...
    "app.insert_failed": {
        "ru": "Не удалось вставить текст — скопируйте его из окна",
        "en": "Could not insert the text — copy it from the window",
        "de": "Text konnte nicht eingefügt werden — kopieren Sie ihn aus dem Fenster",
        "fr": "Impossible d'insérer le texte — copiez-le depuis la fenêtre",
        "es": "No se pudo insertar el texto: cópialo desde la ventana",
        "zh": "无法插入文本——请从窗口中复制",
        "tr": "Metin eklenemedi — pencereden kopyalayın",
        "hy": "Չհաջողվեց տեղադրել տեքստը՝ պատճենեք այն պատուհանից",
    },
