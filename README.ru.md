...
на этой странице в разделе **Assets** возьми файл, оканчивающийся на
**`-Installer.exe`**, и запусти двойным кликом: через минуту WinDictoo
появится в меню «Пуск» и на рабочем столе. Ни терминала, ни прав
администратора, ни Python — внутри уже есть всё. Обновления ставятся так же
поверх (или прямо из приложения: Настройки → Приватность → «Проверить
обновления»).
