...
Там же лежат варианты без установки — для тех, кому так удобнее:

- **`WinDictoo-<версия>-portable.exe`** — один самодостаточный файл:
  запустил и диктуешь; стартует медленнее, т.к. распаковывает себя при
  каждом запуске.
- **`WinDictoo-<версия>-win64.zip`** — то же папкой: распакуй и запускай
  `WinDictoo.exe` внутри.

Единственная докачка — модель распознавания (~500 МБ) при первом запуске.
