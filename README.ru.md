...
## Скачать и запустить (без установки)

**[⬇ Скачать последний релиз](https://github.com/nowoandi/WinDictoo/releases/latest)** —
готовые сборки, не нужны ни Python, ни сборка из исходников:

- **`WinDictoo-<версия>-portable.exe`** — один самодостаточный файл:
  скачал, запустил двойным кликом, диктуешь. Ничего не устанавливается;
  при каждом старте файл распаковывает себя, поэтому запуск занимает
  чуть больше времени.
- **`WinDictoo-<версия>-win64.zip`** — то же самое папкой: распакуй куда
  удобно и запускай `WinDictoo.exe` внутри. Стартует заметно быстрее —
  рекомендуется для постоянной работы.

Внутри уже есть всё (среда Python, движок Whisper, интерфейс). Единственная
докачка — модель распознавания (~500 МБ) при первом запуске.
...
- Windows 10/11; ~500 МБ на модель `small` (скачивается автоматически при
  первом запуске).
